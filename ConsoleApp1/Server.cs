...
public class GameServer : IGameServer
{
    ...
    public static GameServer Connect(string gameCode)
    {
        lock (activeServers)
        {
            if (activeServers.ContainsKey(gameCode))
            {
                GameServer server = activeServers[gameCode];
                server.AcceptNewClient();
                return server;
            }
            else
            {
                Console.WriteLine($"No active game found with code {gameCode}. Starting a new server.");
                GameServer newServer = new GameServer(gameCode);
                activeServers[gameCode] = newServer;
                newServer.StartServer();
                return newServer;
            }
        }
    ...
    {
        // This method will be called when a new client tries to join an existing game
        lock (clientLock)
        {
            if (playerCount < MaxPlayers)
            {
                playerCount++;
                Console.WriteLine($"New client joined. Total players: {playerCount}/{MaxPlayers}");
                // Notify the new client of the current game code and start the game once all players are connected
                StartGameIfReady();
            }
            else
            {
