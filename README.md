...
1.2 Description
•	GuessMate is an interactive, turn-based game where players upload images and provide hints for others to guess. The game currently supports single-player mode, where players compete against the computer. The game includes multiple stages: choosing the number of players, entering names, selecting a game theme, uploading images, gameplay, and displaying final scores. The system includes two databases—one for computer-selected images and another for player-uploaded images. The multiplayer logic is planned for future updates.
...
• 	Data flow diagrams will show the interactions between the MainWindow, GameLobby, PlayerTurn, PlayGround, and databases. The client-server communication logic is responsible for sending high score data to the server, ensuring smooth gameplay and data retrieva
...
•	FinalScores: Displays final player scores after the game ends.
